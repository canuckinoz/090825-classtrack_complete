...
export default function CentralLogin() {
  ...
  const [email, setEmail] = useState('district-a@example.com');
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setErr('');
    try {
      const res = await fetch('/api/auth/login/central', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-purple-50">
      <form
        onSubmit={onSubmit}
      ...
      >
        ...
        />
        <button className="w-full bg-purple-700 hover:bg-purple-800 text-white rounded px-3 py-2">
          Sign in
        </button>
      </form>
    </div>
