...
__export(src_exports, {
  buildHourlyBuckets: () => buildHourlyBuckets,
  computeABCClassInsights: () => computeABCClassInsights,
  computeABCStudentInsights: () => computeABCStudentInsights,
  computeClassForecast: () => computeClassForecast,
  computeClassWeatherMetaphor: () => computeClassWeatherMetaphor,
  computeStudentPrediction: () => computeStudentPrediction,
  riskToMetaphor: () => riskToMetaphor
});
...
function labelForHotspotHour(hour, topTag) {
  if (hour === 14)
    return `${hourReadable(hour)} \u2022 post-lunch`;
  if (hour === 10)
    return `${hourReadable(hour)} \u2022 mid-morning transition`;
  if (topTag)
...
function computeClassForecast(logs, options = {}) {
  ...
  for (const row of logs || []) {
    ...
    if (!d)
      continue;
    if (d < since || d > /* @__PURE__ */ new Date())
      continue;
    const hour = toHour(d);
    if (hour < 0 || hour > 23)
      continue;
  ...
  const hotspotPool = hourRisk.filter((r) => r.samples >= minSamplesPerHour && (!allow || allow.has(r.hour)));
  ...
  const focus_students = [...byStudent.entries()].map(([id, s]) => {
    const total = s.pos + s.neg;
    const risk = total > 0 ? s.neg / (total + EPSILON) : 0;
    return { student_id: id, risk, samples: total };
  }).filter((x) => x.samples >= minSamplesPerStudent).sort((a, b) => b.risk - a.risk || b.samples - a.samples).slice(0, 5);
  return {
    risk_level,
...
function computeStudentPrediction(student_id, behaviours, options) {
  var _a, _b;
  const positive = new Set(((_a = options == null ? void 0 : options.positiveTypes) != null ? _a : DEFAULT_POSITIVE).map(toLower));
  const negative = new Set(((_b = options == null ? void 0 : options.negativeTypes) != null ? _b : DEFAULT_NEGATIVE).map(toLower));
  ...
  if (total < 3)
    suggestions.push("Collect a few more observations this week");
  if (risk_level >= 3)
...
function buildHourlyBuckets(logs, positiveTypes = DEFAULT_POSITIVE, negativeTypes = DEFAULT_NEGATIVE) {
  const posSet = new Set(positiveTypes.map(toLower));
  const negSet = new Set(negativeTypes.map(toLower));
...
var DEFAULT_POSITIVE2 = ["Participation", "Helping Others", "Problem Solving", "On Task", "Peer Support"];
var DEFAULT_NEGATIVE2 = ["Calling Out", "Off Task", "Refusal", "Aggression", "Disruption", "Elopement"];
...
function computeABCClassInsights(events, opts = {}) {
  ...
  for (const e of (Array.isArray(events) ? events : [events]) || []) {
    ...
    if (!d || d < since || d > /* @__PURE__ */ new Date())
      continue;
    ...
    if (A)
      antCount.set(A, (antCount.get(A) || 0) + w);
    if (C)
      conCount.set(C, (conCount.get(C) || 0) + w);
    if (A) {
      if (!ab.has(A))
        ab.set(A, /* @__PURE__ */ new Map());
      const m = ab.get(A);
      m.set(B, (m.get(B) || 0) + w);
    }
    ...
    if (isPos)
      hourContext[hour].pos += w;
    if (isNeg)
      hourContext[hour].neg += w;
    if ((_a = e.tags) == null ? void 0 : _a.length) {
      for (const t of e.tags) {
        const key = t.toLowerCase().trim();
        if (!key)
          continue;
        if (!tagContext.has(key))
          tagContext.set(key, { pos: 0, neg: 0 });
  ...
  const toSortedRows = (m) => [...m.entries()].map(([name, count]) => ({ name, count, rate: count / (total + EPS) })).sort((a, b) => b.count - a.count);
  ...
  for (const [b, c] of behCount)
    pB.set(b, (c + 1) / (total + behCount.size));
  const abPairs = [];
  ...
  const hourHotspots = hourContext.map((bin, h) => {
    const n = bin.pos + bin.neg;
    const r = n > 0 ? bin.neg / (n + EPS) : 0;
    return { key: hourKey(h), negRate: Number(r.toFixed(3)), delta: Number((r - baselineNeg).toFixed(3)), support: n, kind: "hour" };
  }).filter((x) => x.support > 0).sort((a, b) => b.delta - a.delta || b.support - a.support).slice(0, topKContexts);
  const tagHotspots = [...tagContext.entries()].map(([k, bin]) => {
    const n = bin.pos + bin.neg;
    const r = n > 0 ? bin.neg / (n + EPS) : 0;
    return { key: k, negRate: Number(r.toFixed(3)), delta: Number((r - baselineNeg).toFixed(3)), support: n, kind: "tag" };
  }).filter((x) => x.support >= 3).sort((a, b) => b.delta - a.delta || b.support - a.support).slice(0, topKContexts);
  const focusStudents = [...byStudent.entries()].map(([id, s]) => {
    const n = s.pos + s.neg;
    const share = n > 0 ? s.neg / (n + EPS) : 0;
    const avgInt = s.intensityN > 0 ? s.intensitySum / s.intensityN : void 0;
    const suggestions = [];
    if (share >= 0.7)
      suggestions.push("Immediate co-regulation; shorten task with choice of two");
    else if (share >= 0.5)
      suggestions.push("Prime with role; schedule short movement breaks");
    if ((avgInt || 0) >= 3)
      suggestions.push("Plan calm-start + de-escalation script");
    return { studentId: id, negShare: Number(share.toFixed(3)), samples: n, avgIntensity: avgInt ? Number(avgInt.toFixed(2)) : void 0, suggestions };
  }).filter((s) => s.samples >= minSamplesPerStudent).sort((a, b) => b.negShare - a.negShare || b.samples - a.samples).slice(0, 10);
  return {
    summary: {
      total,
      windowDays,
...
function computeABCStudentInsights(studentId, events, opts = {}) {
  var _a;
  const { windowDays = DEFAULT_WINDOW_DAYS, positiveBehaviors = DEFAULT_POSITIVE2, negativeBehaviors = DEFAULT_NEGATIVE2 } = opts;
  const since = new Date(Date.now() - windowDays * 24 * 60 * 60 * 1e3);
  const posSet = new Set(positiveBehaviors.map((b) => b.toLowerCase()));
  const negSet = new Set(negativeBehaviors.map((b) => b.toLowerCase()));
  ...
  for (const e of events || []) {
    ...
    if (A)
      ant.set(A, (ant.get(A) || 0) + 1);
    if (C)
      con.set(C, (con.get(C) || 0) + 1);
  }
  const negShare = total > 0 ? neg / (pos + neg + EPS) : 0;
  const toRows = (m) => [...m.entries()].map(([name, count]) => ({ name, count })).sort((a, b) => b.count - a.count).slice(0, 5);
  const hi = hours.map((bin, h) => {
    const n = bin.pos + bin.neg;
    const r = n > 0 ? bin.neg / (n + EPS) : 0;
    return { key: hourKey(h), negRate: Number(r.toFixed(3)), delta: 0, support: n, kind: "hour" };
  ...
  if (((_a = hi[0]) == null ? void 0 : _a.negRate) >= 0.6)
    suggestions.push(`Plan supports around ${hi[0].key} (timer, chunking, movement cue)`);
  if (total < 3)
    suggestions.push("Collect a few more observations this week");
  return {
    studentId,
