function bad(code){ const e = new Error(code); e.status = 400; return e; }
function forbidden(code){ const e = new Error(code); e.status = 403; return e; }
...
function ensureClassInScope(user, classId){
  ...
  const ok = Array.isArray(user.scope?.classIds) && user.scope.classIds.includes(classId);
